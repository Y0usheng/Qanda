...
* Because the API of `GET /threads` returns 5 elements at a time, you don't need to display the full list one at a time. Section `2.6.1` covers how you can get some extra marks for doing this with an infinite scroll, but for `2.2` to get the marks all you need to do is have a `more` button that each time it's clicked, it shows the next 5 elemetns until no more items are left, then the button disappears.

#### 2.3. Individual thread screen
...
* This individual thread screen should include the list of threads on the left (`2.2`) but the main page body content should contain information on threads that includes:
  * Title
  * Body content
  * Number of likes
* This page will later on include things like edit, delete, like, watch, comments etc, but you can skip this for `2.3`.

### Milestone 3 - Thread Interactions

Milestone 3 focuses on how to interact with threads once they've been made

#### 3.1. Editing a thread
...
### Milestone 6 - Challenge Components (`advanced`)
...
### Milestone 7 - Very Challenge Components (`advanced *= 2`)

#### 7.1. Static feed offline access
...
#### 7.2 Fragment based URL routing
